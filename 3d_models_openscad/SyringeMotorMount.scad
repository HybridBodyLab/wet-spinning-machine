...
module nema_17_mount()
{
    ...
    //these are for the smooth rod holder - Megan
    holderHeight = 10;
    holderThickness = 2;
	...
	translate([-frame_width/2, 0, 0])
	{
		...
		translate([0,-screwPlateSize,-wall_thickness]){
			difference(){
				cube(size=[frame_width,frame_width+2*screwPlateSize,wall_thickness]);
                ...
                //top screw plate added - Megan
                translate([frame_width/2+inch/2, frame_width+screwPlateSize*(3/2), 0]){
					cylinder(h=wall_thickness, r=screwRadius);
				}
                translate([frame_width/2-inch/2, frame_width+screwPlateSize*(3/2), 0]){
					cylinder(h=wall_thickness, r=screwRadius);
    ...
    // motion shaft holder - Megan
    translate([0, -(screwPlateSize - holderHeight), smoothAxisHeight - wall_thickness])rotate([90,0,0]){
        difference(){
            union(){
                cylinder(h = holderHeight, r = smoothRodRadius+holderThickness);
                translate([-(smoothRodRadius + holderThickness), -(smoothAxisHeight-wall_thickness), 0]){
                    cube([2*(smoothRodRadius + holderThickness), smoothAxisHeight-wall_thickness, holderHeight]);
                }
            }
            cylinder(h = holderHeight, r = smoothRodRadius);
        }
    }
